...
@Component({
  selector: 'my-chip-autocomplete',
  templateUrl: './my-chip-autocomplete.component.html',
  styleUrls: ['./my-chip-autocomplete.component.scss'],
  changeDetection: ChangeDetectionStrategy.OnPush,
  providers: [
    {
      provide: NG_VALUE_ACCESSOR,
      useExisting: forwardRef(() => MyChipAutocompleteComponent),
      multi: true
    }
  ]
})
export class MyChipAutocompleteComponent implements OnInit, ControlValueAccessor {
  ...
  filterOption(key: string) {
    return (key === '') ? this.options : this.options.filter(f =>
      this.isOptionString ? f.toLowerCase().includes(key.toLowerCase()) :
        (f[this.displayWith]).toString().toLowerCase().includes(key.toLowerCase()));
  }
